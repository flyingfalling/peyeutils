...
def main():
    wajd_idx=args.wajd_idx;
    wajd_gaze=args.wajd_gaze;
    orig_idx=args.orig_idx;
    orig_gaze=args.orig_gaze;

    marmoskip=args.marmoskip;
    ...
    for count in range(repeatnum):
        ...
        corrdf.to_csv('gazecorrs'+str(count)+'_'+str(inf_error_sigma)+'.csv', index=False);

        ncorrdf = pd.DataFrame( ncorrlist );
        ncorrdf.to_csv('nullcorrs'+str(count)+'_'+str(inf_error_sigma)+'.csv', index=False);
        
        distdf = pd.concat( distlist );
        distdf.to_csv('gazedists'+str(count)+'_'+str(inf_error_sigma)+'.csv', index=False);
